...
export default function Home() {
  ...
  const handleConsentAccept = () => {
    ...
    pushNotification(`🚀 Welcome aboard, ${displayName.trim()}!`);
  };

  // === SOCKET.IO SETUP ===
  useEffect(() => {
    if (showConsent) return; // chỉ kết nối sau khi đồng ý điều khoản

    fetch("/api/socket_io"); // Kích hoạt server nếu chưa
    ...
    socket.on("connect", () => {
      ...
      // Gửi tên hiển thị lên server
      socket.emit("user:join", { name: displayName });
    });

    socket.on("user:join", (user) => {
      pushNotification(`🪐 Explorer joined: ${user.displayName}`);
    });

    socket.on("user:leave", (user) => {
      pushNotification(`👋 Explorer left: ${user.id}`);
    });
